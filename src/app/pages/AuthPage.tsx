    ...
    <div className="w-full bg-[#fffeff] flex h-[44px] items-center justify-between px-4 py-2 shrink-0">
      ...
      <div className="flex gap-[2px] items-center shrink-0">
        ...
        <div className="h-[15.385px] relative w-[24.038px]">
          ...
          >
            <path
              d={STATUS_BAR_PATHS.batteryOutline}
              opacity="0.35"
              stroke="#9D94AA"
              strokeOpacity="0.4"
            ...
            />
            <path
              d={STATUS_BAR_PATHS.batteryNode}
              fill="#1A1128"
              opacity="0.4"
            />
  ...
  return (
    <div className="flex gap-3 items-center w-full">
      {Array.from({ length: total }).map((_, i) => (
        <div
          key={i}
          className="flex-1 h-1 min-w-0 overflow-clip rounded-full relative"
        >
          <div
            className={`absolute inset-0 rounded-full ${
              i < filled
                ? "bg-[#7d3aea]"
                : "bg-[#c9b4f6] border border-[#e2d9ef]"
            }`}
          />
        </div>
      ))}
    ...
    <div className="flex flex-col gap-8 w-full">
      <div className="flex flex-col gap-3 w-full">
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <ScreenSubtitle>
            In a few moments you will be ready to share
          </ScreenSubtitle>
        </div>
        <div className="flex gap-3 items-center w-full">
  ...
  return (
    <div className="flex flex-col gap-8 w-full">
      <div className="flex flex-col gap-6 w-full">
        <button
          onClick={onBack}
          className="flex items-center p-2 -ml-2 cursor-pointer"
          aria-label="Go back"
        >
    ...
    <div className="min-h-screen bg-[#f0ecf7] flex items-start justify-center">
      <div className="w-full max-w-[800px] min-w-0 bg-[#fffeff] flex flex-col min-h-screen">
        ...
        <div className="flex-1 px-4 pt-8 pb-4 overflow-y-auto">
          <div className="w-full max-w-[800px] mx-auto">{screens[step]}</div>
        </div>
        <HomeIndicator />
      </div>
    </div>
