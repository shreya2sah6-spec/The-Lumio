...
function SettingsItem({ icon: IconComponent, label, onClick, isDestructive }: { icon: Icon; label: string; onClick?: () => void; isDestructive?: boolean }) {
  return (
    <button
      onClick={onClick}
      className="flex gap-[12px] items-center py-[12px] w-full border-b border-[rgba(157,148,170,0.4)] cursor-pointer"
    >
      <IconComponent size={24} weight="regular" color={isDestructive ? "#DE3226" : "#1A1128"} />
      <div className="flex-1 font-['Manrope',sans-serif] font-medium text-[16px] leading-[25px] tracking-[0.16px] text-left">
        <p className={isDestructive ? "text-[#de3226]" : "text-[#1a1128]"}>{label}</p>
      </div>
      {!isDestructive && (
    ...
    <div className="min-h-screen bg-[#fffeff] flex items-start justify-center">
      <div className="w-full max-w-[800px] min-w-[360px] bg-[#fffeff] flex flex-col min-h-screen">
        ...
        <div className="bg-[#fffeff] flex items-center px-[16px] py-[12px]">
          <button className="p-[8px] cursor-pointer flex items-center" onClick={() => navigate("/profile")}>
            <ArrowLeft size={24} color="#1A1128" />
          </button>
          <h1 className="font-['Roboto_Serif',serif] font-semibold text-[#1a1128] text-[24px] leading-[31px] ml-[12px]">
            Settings
          </h1>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-[16px] py-[20px]">
          {/* Your account */}
          <div className="flex flex-col gap-[20px] mb-[20px] border-b border-[rgba(157,148,170,0.4)] pb-[20px]">
            <h2 className="font-['Roboto_Serif',serif] font-semibold text-[#1a1128] text-[20px] leading-[28px]">
              Your account
            </h2>
