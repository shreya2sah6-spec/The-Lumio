...
import {
  MagnifyingGlass, Funnel, VideoCamera, House, Briefcase, SuitcaseSimple,
  Plus, Users,
...
const topRatedMentors: Mentor[] = [
  { id: "m1", name: "Shruti Jain", title: "Sr. Fashion Designer", company: "MAX", avatar: imgMentor1, experience: "7 yrs exp • EX - ZARA", rating: 4.9, reviews: 120, originalPrice: 600, discountedPrice: 300, isTopMentor: true },
  { id: "m2", name: "Priya Mehta", title: "Sr. Fashion Designer", company: "MAX", avatar: imgMentor2, experience: "7 yrs exp • EX - Fab India", rating: 4.7, reviews: 120, originalPrice: 600, discountedPrice: 400, isTopMentor: true },
  { id: "m3", name: "Ravi Kumar", title: "Sr. Fashion Designer", company: "Myntra", avatar: imgMentor3, experience: "8 yrs exp • EX - H&M", rating: 4.7, reviews: 120, originalPrice: 550, discountedPrice: 300, isTopMentor: true },
  { id: "m4", name: "Sneha Patel", title: "Lead Fashion Designer", company: "Anita Dongre", avatar: imgMentor4, experience: "10 yrs exp • EX - Mango", rating: 4.6, reviews: 75, originalPrice: 580, discountedPrice: 250, isTopMentor: true },
];

const pickedForYouMentors: Mentor[] = [
  { id: "m5", name: "Amit Sharma", title: "Mid-Level Designer", company: "Biba", avatar: imgMentor5, experience: "5 yrs exp • EX - Forever 21", rating: 4.4, reviews: 95, originalPrice: 520, discountedPrice: 350 },
  { id: "m6", name: "Neha Verma", title: "Fashion Designer", company: "W for Woman", avatar: imgMentor6, experience: "6 yrs exp • EX - Lifestyle", rating: 4.5, reviews: 88, originalPrice: 480, discountedPrice: 320 },
  { id: "m7", name: "Vikram Singh", title: "Mid-Level Designer", company: "Biba", avatar: imgMentor7, experience: "3 yrs exp • EX - Shein", rating: 4.2, reviews: 80, originalPrice: 100, discountedPrice: "Free" },
  { id: "m8", name: "Anjali Nair", title: "Fashion Designer", company: "Fabindia", avatar: imgMentor8, experience: "4 yrs exp • EX - Westside", rating: 4.3, reviews: 65, originalPrice: 450, discountedPrice: 280 },
];

const additionalMentors: Mentor[] = [
  { id: "m9", name: "Rajesh Patel", title: "Sr. Fashion Designer", company: "Lifestyle", avatar: imgMentor1, experience: "9 yrs exp • EX - Shoppers Stop", rating: 4.8, reviews: 110, originalPrice: 650, discountedPrice: 400 },
  { id: "m10", name: "Kavita Reddy", title: "Lead Designer", company: "FabIndia", avatar: imgMentor2, experience: "11 yrs exp • EX - Good Earth", rating: 4.7, reviews: 98, originalPrice: 700, discountedPrice: 450 },
];
...
function TopBar({ onFilterClick, searchPlaceholder }: { onFilterClick: () => void; searchPlaceholder: string }) {
  return (
    <div className="sticky top-0 z-10 bg-[#fffeff]">
      <StatusBar />
      <div className="flex gap-3 items-center px-4 py-3 h-[64px]">
        <div className="flex-1 bg-white rounded-[4px] border border-[rgba(157,148,170,0.4)] flex items-center gap-2 px-4 py-1 h-10">
...
function MainTabs({ activeTab, onTabChange }: { activeTab: MainTab; onTabChange: (tab: MainTab) => void }) {
  return (
    <div className="w-full border-b border-[#e2d9ef] px-4 pt-4">
      <div className="flex">
        ...
        >
          ...
          <div className={`h-[2px] w-full rounded-tl-[2px] rounded-tr-[2px] ${activeTab === "Discover" ? "bg-[#7d3aea]" : "bg-transparent"}`} />
        </button>
        <button
        ...
        >
          ...
          <div className={`h-[2px] w-full rounded-tl-[2px] rounded-tr-[2px] ${activeTab === "Upcoming Sessions" ? "bg-[#7d3aea]" : "bg-transparent"}`} />
        </button>
      </div>
    </div>
  );
...
function SessionTabs({ activeTab, onTabChange }: { activeTab: SessionTab; onTabChange: (tab: SessionTab) => void }) {
  return (
...
function SessionCard1On1({ session, isExpanded, onToggle }: { session: Session; isExpanded: boolean; onToggle: () => void }) {
    ...
    <div className="bg-white border border-[#e2d9ef] rounded-[4px] overflow-hidden w-full max-w-[768px] min-w-[320px]">
      {isExpanded ? (
        <div className="flex flex-col gap-3 items-start px-4 py-3">
          <button onClick={onToggle} className="flex gap-2 items-center w-full cursor-pointer">
            <div className="flex flex-col gap-2 items-start flex-1">
              <div className={`flex items-center justify-center px-2 py-2 rounded-[2px] ${
                session.category === "Learning" ? "bg-[#f4f7ff]" : "bg-[#d6f5dd]"
              }`}>
              ...
              <p className="font-['Manrope',sans-serif] font-semibold text-[#1a1128] text-[18px] leading-[28px] text-left">
                {isImminent ? `Your session starts in ${session.timeUntil}` : "Session in 5 days"}
              </p>
            </div>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path d="M19.5 8.24988L12 15.7499L4.5 8.24988" stroke="#6B5F7A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          ...
          <div className="flex flex-col gap-3 items-start w-full">
            <div className="flex gap-3 items-center w-full">
              <div className="relative shrink-0 size-12 rounded-[24px] overflow-hidden border border-[#e2d9ef] shadow-[-2px_0px_2px_rgba(200,192,212,0.6)]">
                <img src={session.mentorAvatar} alt={session.mentor} className="absolute inset-0 size-full object-cover" />
              </div>
        ...
        <button onClick={onToggle} className="flex gap-2 items-center px-4 py-3 w-full cursor-pointer">
          <div className="flex flex-col gap-2 items-start flex-1">
            <div className={`flex items-center justify-center px-2 py-2 rounded-[2px] ${
              session.category === "Learning" ? "bg-[#f4f7ff]" : "bg-[#d6f5dd]"
            }`}>
          ...
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path d="M19.5 15.7499L12 8.24988L4.5 15.7499" stroke="#6B5F7A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
      )}
    </div>
...
function SessionCardWebinar({ session, isExpanded, onToggle }: { session: Session; isExpanded: boolean; onToggle: () => void }) {
  return (
    <div className="bg-white border border-[#e2d9ef] rounded-[4px] overflow-hidden w-full max-w-[768px] min-w-[320px]">
      {isExpanded ? (
        <div className="flex flex-col gap-3 items-start px-4 py-3">
              ...
              <div className="flex items-center shrink-0 pointer-events-none shadow-[-2px_0px_2px_rgba(200,192,212,0.6)]">
                <div className="mr-[-12px] relative rounded-[24px] shrink-0 size-12">
                  <div className="absolute inset-0 overflow-hidden rounded-[24px]">
                    <img alt="" className="absolute inset-0 size-full object-cover" src={imgSession1} />
                  </div>
                  <div aria-hidden="true" className="absolute border border-[#e2d9ef] border-solid inset-0 rounded-[24px]" />
                </div>
                <div className="mr-[-12px] relative rounded-[200px] shrink-0 size-12">
                  <div className="absolute inset-0 overflow-hidden rounded-[200px]">
                    <img alt="" className="absolute inset-0 size-full object-cover" src={imgSession2} />
                  </div>
    ...
    <div className="sticky bottom-0 z-10 bg-white shadow-[0px_-1px_2px_rgba(200,192,212,0.6)] flex flex-col items-center">
      <div className="w-full flex items-center justify-around px-4 h-[68px]">
        ...
        <button className="flex flex-col gap-px items-center justify-center h-[50px] px-1 cursor-pointer" onClick={() => navigate("/jobs")}>
          <SuitcaseSimple size={24} color="#6B5F7A" />
  ...
  return (
    <div className="min-h-screen bg-[#f0ecf7] flex items-start justify-center">
      <div className="w-full max-w-[800px] min-w-[360px] bg-[#fffeff] flex flex-col min-h-screen">
        <TopBar
          onFilterClick={() => {}}
          searchPlaceholder={mainTab === "Discover" ? "Search mentors, skills, domain" : "Search Sessions"}
        />
