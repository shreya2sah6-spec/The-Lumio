...
import {
  MagnifyingGlass, BellSimple, ChatCenteredDots, BookmarkSimple,
  CaretLeft, Eye, House, Briefcase, SuitcaseSimple, Plus, Users,
  Repeat, HandsClapping, Check, ShareFat,
  ...
  return (
    <div className="sticky top-0 z-10 bg-[#fffeff] shadow-[0px_1px_2px_rgba(200,192,212,0.4)]">
      ...
      <div className="flex gap-3 items-center px-4 py-3">
        <div className="flex-1 bg-white rounded-[4px] border border-[rgba(157,148,170,0.4)] flex items-center gap-2 px-4 py-2">
          <MagnifyingGlass size={16} color="#6B5F7A" />
          <span className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[14px] leading-[21px]">Search</span>
        </div>
        <button className="p-2 cursor-pointer" onClick={() => navigate("/notifications")}>
          <BellSimple size={24} color="#6B5F7A" />
        </button>
        <button className="p-2 cursor-pointer" onClick={() => navigate("/messages")}>
...
function SaveBtn({ color = "#6B5F7A" }: { color?: string }) {
  ...
  return (
    <button className="p-2 cursor-pointer shrink-0" onClick={() => setSaved((s) => !s)}>
      <BookmarkSimple size={24} color="#6B5F7A" weight={saved ? "fill" : "regular"} />
    </button>
  );
}
...
function ProfileCompletionCard({ onComplete }: { onComplete: () => void }) {
  return (
    <div className="bg-[#f5f0ff] w-full flex flex-col gap-4 px-4 py-3">
      ...
      <button onClick={onComplete} className="bg-white border border-[#7d3aea] rounded-[8px] h-12 w-full flex items-center justify-center cursor-pointer">
...
function PostCard({
  image, avatarSingle, avatarPrimary, avatarSecondary,
  name, role, status, caption, date,
  engagementAvatar, engagementLabel, engagementIcon,
  coCreate, coCreateWith,
}: PostCardProps) {
  return (
          ...
          <div className="absolute bottom-0 left-0 p-4">
            <div className="bg-white rounded-[8px] flex gap-2 items-center p-2 h-[46px]">
              <div className="relative shrink-0">
                <img src={engagementAvatar} alt="" className="size-6 rounded-full object-cover" />
        ...
        <div className="flex gap-1 items-start w-full">
          <div className="flex gap-3 items-center flex-1 min-w-0">
            <div className="relative shrink-0 size-12">
              {avatarSingle ? (
                <img src={avatarSingle} alt="" className="size-full rounded-full object-cover" />
              ) : (
                <>
                  <img src={avatarPrimary} alt="" className="absolute top-0 left-0 size-8 rounded-tl-full rounded-tr-full rounded-bl-full object-cover border border-[#e2d9ef]" />
                  <img src={avatarSecondary} alt="" className="absolute bottom-0 right-0 size-8 object-cover border border-white shadow-[-1px_0px_2px_0px_rgba(26,26,26,0.15)] rounded-t-[200px] rounded-bl-[200px] rounded-br-[0px]" />
                </>
              )}
            </div>
            <div className="flex flex-col flex-1 min-w-0">
              <p className="font-['Manrope',sans-serif] font-medium text-[#1a1128] text-[16px] leading-[25px] tracking-[0.16px] truncate">
                {coCreate ? (
                  <>
                    {name}
                    <span className="font-normal text-[#433059] text-[12px] tracking-[0.24px]"> co-create with </span>
                    {coCreateWith}
                  </>
                ) : name}
              </p>
              <p className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[14px] leading-[21px] truncate">{role}</p>
              {status && (
                <p className="font-['Manrope',sans-serif] font-normal text-[#9d90ad] text-[12px] leading-[18px] tracking-[0.24px] truncate">{status}</p>
              )}
            </div>
          </div>
          <SaveBtn />
        </div>
        <p className="font-['Manrope',sans-serif] font-normal text-[#1a1128] text-[14px] leading-[21px]">{caption}</p>
        <p className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[12px] leading-[18px] tracking-[0.24px] text-right">{date}</p>
      </div>
    </div>
  );
}
...
const creativeCards = [
  { image: imgCreativeCard01, caption: "Floral fashion makes its bold comeback.", views: "2.5k", title: "Floral Fashion", description: "The season's most talked-about trend returns with vibrant botanicals and bold silhouettes dominating every runway." },
  { image: imgCreativeCard02, caption: "Maximalist glamour is everywhere this season.", views: "2.1k", title: "Maximalist Glamour", description: "More is more — sequins, fringe, and oversized drama are rewriting the rules of evening wear for 2026." },
  { image: imgCreativeCard03, caption: "Men's Footwear Trends Unveiled at Pitti Uomo's Fall 2026 Edition.", views: "1.4k", title: "Pitti Uomo Footwear", description: "Classic brogues, chunky platforms, and architectural soles were the story at Florence's most important menswear event." },
  { image: imgCreativeCard04, caption: "Jackets redefine fashion now.", views: "952", title: "The Jacket Moment", description: "From tailored blazers to deconstructed outerwear — the jacket is having its biggest cultural moment in decades." },
];

// ─── Story overlay ────────────────────────────────────────────────────────────


function StoryOverlay({ index, onClose }: { index: number; onClose: () => void }) {
      ...
      <div className="relative h-full w-full max-w-[800px] flex flex-col">
            ...
            {creativeCards.map((_, i) => (
              ...
              >
                <div
                  ...
                  style={{
                    width: i < current ? "100%" : i === current ? `${Math.min(progress, 100)}%` : "0%",
        ...
        >
          ...
          <button
            ...
            style={{ padding: "8px", borderRadius: "200px", background: "#FFFFFF" }}
          >
...
function CreativeMomentumSection({ onOpenStory }: { onOpenStory: (i: number) => void }) {
  return (
        ...
        {creativeCards.map(({ image, caption, views }, i) => (
          ...
          >
            ...
            <div className="-translate-y-1/2 absolute aspect-[240/300] left-0 right-0 top-1/2">
              <img src={image} alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" />
            </div>
            {/* caption at bottom */}
            ...
            <div className="absolute bg-[rgba(26,26,26,0.6)] flex gap-2 items-center left-[148px] px-[8px] py-[4px] rounded-[4px] top-[16px]">
              ...
              <span className="font-['Manrope',sans-serif] font-normal text-white text-[16px] leading-[24px]">{views}</span>
            </div>
          </button>
        ))}
      </div>
    ...
    <div className="bg-white w-full flex flex-col pt-2">
      <div className="overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        <div className="flex gap-3 px-4" style={{ width: "max-content" }}>
          {[imgPromoImage, imgPromoImage1, imgPromoImage2, imgPromoImage3].map((src, i) => (
            <div key={i} className="h-[291px] w-[292px] shrink-0 rounded-[4px] overflow-hidden">
      ...
      <div className="flex flex-col gap-3 px-4 pt-3 pb-6">
        <div className="flex gap-1 items-start w-full">
          <div className="flex gap-3 items-center flex-1 min-w-0">
            <img src={imgBrandLogo} alt="" className="size-12 rounded-full object-cover shrink-0" />
            <div className="flex flex-col flex-1 min-w-0">
              <p className="font-['Manrope',sans-serif] font-medium text-[#1a1128] text-[16px] leading-[25px] tracking-[0.16px] truncate">Manish Malhotra</p>
              <p className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[14px] leading-[21px] truncate">Job Opportunity</p>
            </div>
          </div>
          <SaveBtn />
        </div>
        ...
        >
          ...
          <span className={`font-['Manrope',sans-serif] font-semibold text-[16px] leading-[20px] tracking-[0.48px] ${applied ? "text-[#208436]" : "text-white"}`}>
            {applied ? "Applied" : "Quick Apply"}
          </span>
        </button>
        <p className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[12px] leading-[18px] tracking-[0.24px] text-right">21 July</p>
      </div>
    </div>
  );
}
    ...
    <div className="sticky bottom-0 z-10 bg-white shadow-[0px_-1px_2px_rgba(200,192,212,0.6)] flex flex-col items-center">
      <div className="w-full flex items-center justify-around px-4 h-[68px]">
        ...
        <button className="flex flex-col gap-px items-center justify-center h-[50px] px-4 cursor-pointer" onClick={() => navigate("/home/feed")}>
          <House size={24} weight="fill" color="#1A1128" />
          <span className="font-['Manrope',sans-serif] font-medium text-[#1a1128] text-[14px] leading-[21px] tracking-[0.14px]">Home</span>
        </button>
        {/* Jobs */}
