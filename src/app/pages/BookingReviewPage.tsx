...
import { ArrowLeft, CalendarDots, Question, ShieldCheck, VideoCamera } from "@phosphor-icons/react";
import svgPaths from "@/imports/MentorsBookingReview-2/svg-37p92hqhmy";
import imgMentorAvatar from "@/imports/MentorsBookingReview-2/8a0297188511b9e7d739e0bdb0fad1599992ea67.png";
import { BookingConfirmedPage } from "./BookingConfirmedPage";

export function BookingReviewPage({ onBack }: { onBack: () => void }) {
  const [callAbout, setCallAbout] = useState("");
  const [showConfirmed, setShowConfirmed] = useState(false);

  if (showConfirmed) {
    return <BookingConfirmedPage onDone={onBack} />;
  }
  const MAX_CHARS = 500;
  ...
  return (
    <div className="min-h-screen bg-[#fffeff] flex items-start justify-center">
      <div className="w-full max-w-[800px] min-w-[360px] bg-[#fffeff] flex flex-col min-h-screen">
          ...
          <div className="flex gap-[2px] items-center shrink-0">
            ...
            <div className="h-[15.385px] relative w-[24.038px]">
              <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24.0385 15.3846">
                <path d="M3.02599 2.71124H19.0514C20.2019 2.71129 21.1344 3.64466 21.1344 4.79522V10.5638C21.1344 11.7143 20.2019 12.6477 19.0514 12.6478H3.02599C1.8754 12.6478 0.942008 11.7144 0.942008 10.5638V4.79522C0.942008 3.64463 1.8754 2.71124 3.02599 2.71124Z" opacity="0.35" stroke="#9D94AA" strokeOpacity="0.4" strokeWidth="0.961538" fill="none" />
                <path d="M22.5769 5.75643V9.60258C23.3507 9.27684 23.8539 8.51906 23.8539 7.67951C23.8539 6.83996 23.3507 6.08218 22.5769 5.75643" fill="#1A1128" opacity="0.4" />
                <path d="M2.38462 5.4359C2.38462 4.72784 2.95861 4.15385 3.66667 4.15385H18.4103C19.1183 4.15385 19.6923 4.72784 19.6923 5.4359V9.92308C19.6923 10.6311 19.1183 11.2051 18.4103 11.2051H3.66667C2.95861 11.2051 2.38462 10.6311 2.38462 9.92308V5.4359Z" fill="#1A1128" />
              </svg>
            </div>
          </div>
        </div>
        ...
        {/* Header */}
        <div className="sticky top-0 z-20 bg-[#fffeff] px-4 py-3 flex items-center gap-3 shadow-sm shrink-0">
          <button onClick={onBack} className="p-2 -ml-2 shrink-0 flex items-center justify-center">
            <ArrowLeft size={24} color="#1A1128" />
          </button>
          <span className="font-['Roboto_Serif',sans-serif] font-semibold text-[#1a1128] text-[24px] leading-[31px]">
            Review the Booking
          </span>
        </div>
          ...
          <div className="px-4">
            ...
            <div className="flex items-center pb-[12px]">
              ...
              <div className="flex gap-[4px] items-center shrink-0">
                ...
                <span className="font-['Manrope',sans-serif] font-normal text-[#6b5f7a] text-[14px] leading-[21px]">60mins</span>
              </div>
              ...
              <div className="flex items-center flex-1 min-w-px ml-[12px]">
                ...
                <button className="flex items-center justify-center pl-[16px] py-[8px] rounded-[4px] shrink-0">
                  <span className="font-['Manrope',sans-serif] font-semibold text-[#7d3aea] text-[14px] leading-[20px] tracking-[0.14px]">Edit</span>
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-[16px] py-[20px]">
              ...
              <div className="relative bg-white rounded-[8px] h-[140px]">
                ...
                <textarea
                  ...
                  placeholder="Portfolio feedback, career guidance, interview prep, design direction, or anything you'd like support with."
              ...
              <div className="relative rounded-[8px] w-full">
                <div className="flex flex-col overflow-clip rounded-[8px]">
                  ...
                  <div className="bg-[#f7f4fa] relative px-[16px] py-[12px]">
                    ...
                    <span className="font-['Manrope',sans-serif] font-normal text-[#433059] text-[14px] leading-[21px]">Booking summary</span>
                  </div>
                  {/* 1:1 Mentorship */}
                  <div className="bg-white relative px-[16px] py-[12px] flex gap-[12px] items-center">
                    <div aria-hidden className="absolute border-b border-[rgba(157,148,170,0.4)] inset-0 pointer-events-none" />
                    <span className="font-['Manrope',sans-serif] font-normal text-[#2d2040] text-[14px] leading-[21px] flex-1">1:1 Mentorship</span>
                    <span className="font-['Manrope',sans-serif] font-normal text-[#2d2040] text-[14px] leading-[21px] text-right shrink-0">₹280</span>
                  </div>
                  {/* Platform fee */}
                  <div className="bg-white relative px-[16px] py-[12px] flex gap-[12px] items-center">
                    <div aria-hidden className="absolute border-b border-[rgba(157,148,170,0.4)] inset-0 pointer-events-none" />
                    <div className="flex flex-1 gap-[8px] items-center min-w-px">
                      <span className="font-['Manrope',sans-serif] font-normal text-[#2d2040] text-[14px] leading-[21px]">Platform Fee</span>
                      <Question size={16} color="#6B5F7A" />
                    </div>
                    <span className="font-['Manrope',sans-serif] font-normal text-[#2d2040] text-[14px] leading-[21px] text-right shrink-0">₹20</span>
                  </div>
                  {/* Total */}
                  <div className="bg-[#f7f4fa] px-[16px] py-[12px] flex gap-[12px] items-center">
                    <span className="font-['Manrope',sans-serif] font-medium text-[#1a1128] text-[16px] leading-[25px] tracking-[0.16px] flex-1">Total</span>
                    <span className="font-['Manrope',sans-serif] font-medium text-[#1a1128] text-[16px] leading-[25px] tracking-[0.16px] text-right shrink-0">₹300/hr</span>
                  </div>
                </div>
