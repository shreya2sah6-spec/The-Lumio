      ...
      <div className="flex flex-col gap-2 items-start px-4 py-3">
        ...
        <div className="flex gap-3 items-start w-full">
          ...
          {mentor.isTopMentor && (
            <div className="bg-gradient-to-l from-[rgba(247,181,0,0.4)] to-[rgba(254,250,225,0.4)] flex items-center justify-center p-2 rounded-[2px] shrink-0">
              {/* Caption 1 Trim — compact tag label with vertical trim */}
              <span className="type-caption-1-trim text-[#1a1128] whitespace-nowrap">
                Top 1%
              </span>
            </div>
          )}
        ...
        <div className="flex gap-1 h-[18px] items-center w-full">
          ...
          <div className="flex gap-[2px] items-center shrink-0">
            {/* Caption 1 — 12px medium, rating value */}
            <span className="type-caption-1 text-[#433059]">{mentor.rating}</span>
            {/* Caption Regular — 12px regular, subdued count */}
            <span className="type-caption text-[#9d94aa]">({mentor.reviews})</span>
          </div>
        </div>

        {/* ── Name · role · experience ── */}
        <div className="flex flex-col gap-[2px] items-start w-full">
          {/* H4 — primary name emphasis */}
          <p className="type-h4 text-[#1a1128]">{mentor.name}</p>
          {/* Caption Regular — secondary role metadata */}
          <p className="type-caption text-[#433059] overflow-hidden text-ellipsis whitespace-nowrap w-full">
            {mentor.title} @{mentor.company}
          </p>
          {/* Caption Regular — supporting experience metadata */}
          <p className="type-caption text-[#6b5f7a] overflow-hidden text-ellipsis whitespace-nowrap w-full">
            {mentor.experience}
          </p>
        </div>

        {/* ── Pricing row + bookmark ── */}
            ...
            <div className="flex items-center">
              ...
              <span className="type-h5 text-[#1a1128]">
                {mentor.discountedPrice === "Free"
                  ? "Free"
                  : `₹${mentor.discountedPrice}`}
              </span>
              {mentor.discountedPrice !== "Free" && (
                /* Caption Regular — pricing unit, subdued */
                <span className="type-caption text-[#6b5f7a]">hr</span>
              )}
            </div>
          </div>
          <SaveButton />
